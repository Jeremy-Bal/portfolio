...
export default class Asteroide
{
    ...
    addToScene()
    {
        ...
        //SET SCENE ENV
        //this.scene.environment = this.ressources.items.nebula
    }
    ...
    update()
    {
        ...
        this.group.traverse((mesh)=>{
            ...
            {
                mesh.position.x = (Math.sin(angle - mesh.clonePosition.x) * 40) - 10
                mesh.position.z = (Math.cos(angle - mesh.clonePosition.x) * 40) + mesh.clonePosition.z * 0.5
                mesh.position.y = (Math.cos(angle + mesh.clonePosition.y * 100) * 20) - 7
            }
        })
    }
}   